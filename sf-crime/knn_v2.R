#This is a prediction implementing knn method. The predicted probability of each category of the test set is given by the proportion of category in the knn, which the distance is calculated using unified x,y and time. However, the scale of time the coordinates is considered different here. 

#Cross validation will be tested in later ver.
...
train_data <- train %>% 
	mutate(time=(as.numeric(as.POSIXct(Dates)) - min_axis[1]) / diff_axis[1] * 100 * time_cor_ratio,x = (X-min_axis[2])/diff_axis[2] * 100, y = (Y - min_axis[3])/diff_axis[3] * 100) %>%
