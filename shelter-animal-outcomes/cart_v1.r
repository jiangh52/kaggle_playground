...
combi <- subset(combi,select=-c(Name, DateTime, OutcomeSubtype, SexuponOutcome, AgeuponOutcome, Breed))
...
# fit <- train(OutcomeType~., data=train_train, method="rpart",tuneLength=5) #cp=0.003
# cv_pred <- predict(fit,newdata=train_test,type="prob")
# cv_pred_check <- data.frame(Id=1:nrow(cv_pred), cv_pred)
# cv_result <- data.frame(Id=1:nrow(train_test),Category=train_test$OutcomeType)
# log.loss(cv_pred_check,cv_result)
#0.9670216

# fit <- rpart(OutcomeType ~., data=train,control=rpart.control(cp=0.003))
# rpart.plot(fit)
